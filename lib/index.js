...
(function (win, doc) {
    ...
    touchRobot.prototype._getPos = function (type) {
        ...
        var lines = [];
        if (type == 'left') {
            var maxHeight = 200; //垂直方向最大高度
            var startX = randomFrom(rect.x + rect.width/2, rect.x + rect.width);
            var startY = randomFrom(rect.y + rect.height / 3, rect.y + rect.height / 2) ;
            var endX = randomFrom(rect.x, rect.x + rect.width/2);
            var endY = randomFrom(rect.y, rect.y + rect.height);
    ...
    touchRobot.prototype._mouseMoveTo = function (posArray, curIndex, callback){
      var x = posArray[curIndex].split(' ')[0];
      var y = posArray[curIndex].split(' ')[1];
    ...
    touchRobot.prototype._triggerMouseEvent = function (eventType, x, y) {
      // 获取目标元素的坐标、大小
      var rect = this.touchEle.getBoundingClientRect();
      var randomLen = randomFrom()
      var randomX = x || randomFrom(rect.left, rect.left + rect.width);
    ...
    touchRobot.prototype.mouse = function (type) {
        var posArray = touchRobotPos[type] || []
    ...
    touchRobot.prototype._touch = function (type) {
    ...
    // 
    touchRobot.prototype.mouseTo = function (startX, startY, endX, endY) {
        var posArray = getXY(startX, startY, endX, endY, 'horizontal')
        var x = posArray[0].split(' ')[0];
        var y = posArray[0].split(' ')[1];
        this._triggerMouseEvent('mouseend', x, y);
    ...
    touchRobot.prototype.mouseClick = function () {        
        var rect = this.touchEle.getBoundingClientRect();
        var x = rect.x + randomFrom(rect.width/3, rect.width - rect.width/3)
        var y = rect.y + randomFrom(rect.height/3, rect.height - rect.height/3)
        this._triggerMouseEvent('mouseend', x, y);
